...
opts_chunk$set(message = F, fig.align = "center", warning = F, echo = F, fig.width = 15, fig.height = 15)
	...
	for(i in 1 : length(gam1)) {
		...
		for(j in 1 : length(nu1)) {
			...
			for(k in 1 : length(deg1)) {
				type1 <- paste0("Gam=", gam1[i], ", Nu=", nu1[j], ", Deg=", deg1[k]) 
...
plotfun <- function(x, svm_model_out) {
	...
	for(i in 1 : length(svm_modelN)) {
		for(j in 1 : length(svm_modelN[[i]])) {
			...
			pred2 <- c(pred2, predict(svm_modelN[[i]][[j]][[k]]))
			l <- l + 1
			}
		}
	}
